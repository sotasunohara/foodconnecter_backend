...
function dbconnect(request,query,callback){
    ...
    //ストリーミングを許可
    request.stream=true;
    var q="SELECT uinfo.username,food.food,food.foodnum FROM dbo.uinfo,dbo.food WHERE uinfo.id=food.target_id and food.target_id=@id and food.receive IS NULL";
    request.query(q);
